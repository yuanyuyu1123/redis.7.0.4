 ...
 */
//在 little-endian 架构上一次计算多个字节的非反转 CRC。如果需要反转CRC，调用前反转，调用后反转。
// 64 位 crc = 一次处理 8 个字节；
...
/* Return the CRC of buf[0..len-1] with initial crc, processing eight bytes
   ...
   the architecture. */
/**
 * 返回带有初始 crc 的 buf[0..len-1] 的 CRC，使用传入的查找表一次处理 8 个字节。这将根据体系结构的字节顺序选择两个例程之一。
 * */
